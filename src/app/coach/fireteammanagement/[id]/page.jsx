...
export default function CoachFireteamDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      router.push("/coach/fireteammanagement");
  ...
  const openEditExperience = async (exp) => {
    try {
      // Always re-fetch the fireteam to get the latest nested agenda + exhibits.
      // (The list endpoint doesn't return nested data, so cached state is unreliable.)
      const data = await fireteamService.getFireteam(id);
      const ft = data.fireTeam;
  ...
  const handleSaveEditExperience = async () => {
    ...
    try {
      ...
      // Re-fetch full fireteam so experiences state has fresh nested agenda/exhibits
      await fetchDetails();
  ...
  return (
    <div className="h-screen flex bg-[#f5f5f5] overflow-hidden">
      ...
      <main className="flex-1 min-w-0 overflow-y-auto px-10 py-8">
        ...
        {/* Back */}
        <button
          onClick={() => router.push("/coach/fireteammanagement")}
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex flex-col items-end gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <button onClick={() => router.push(`/coach/fireteammanagement/${id}/experience/${exp.id}`)}
